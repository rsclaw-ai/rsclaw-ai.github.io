---
layout: page
title: Download RsClaw (Crab AI)
description: Download RsClaw (Crab AI) desktop app or CLI tool
sidebar: false
---
...
const INSTALL_CMDS = [
  { label: 'macOS / Linux', lang: 'bash', cmd: 'curl -fsSL https://app.rsclaw.ai/scripts/install.sh | bash' },
  { label: 'Windows', lang: 'powershell', cmd: 'irm https://app.rsclaw.ai/scripts/install.ps1 | iex' },
]
...
.dl-tabs button:not(.active):hover { color: var(--dl-text-1); }
...
.dl-main-btn .btn-icon { font-size: 22px; }
.dl-meta { margin-top: 12px; font-size: 12px; color: var(--dl-text-3); font-family: monospace; }
.dl-meta-sub { margin-top: 6px; font-size: 13px; color: var(--dl-text-2); }

.dl-switch { margin-top: 20px; font-size: 13px; color: var(--dl-text-3); }
.dl-switch a { color: #f97316 !important; text-decoration: none !important; cursor: pointer; }
.dl-switch a:hover { text-decoration: underline !important; }

.dl-section { max-width: 860px; margin: 0 auto; padding: 0 24px; }
.dl-section-title {
  font-size: 24px !important; font-weight: 700 !important;
  text-align: center; margin: 60px 0 8px !important;
...
.dl-section-desc { text-align: center; color: var(--dl-text-2); font-size: 14px; margin-bottom: 28px; }
.dl-grid {
  display: grid;
...
.dl-card-icon {
  ...
  background: var(--dl-surface-hover); border-radius: 10px;
}
.dl-card-info { flex: 1; min-width: 0; }
.dl-card-title { font-size: 14px; font-weight: 600; color: var(--dl-text-1); margin-bottom: 2px; }
.dl-card-detail { font-size: 11px; color: var(--dl-text-3); font-family: monospace; }

/* Install section */
.dl-install { max-width: 860px; margin: 48px auto 0; padding: 0 24px; }
.dl-install-box {
  background: var(--dl-surface);
  border: 1px solid var(--dl-border);
...
.dl-install-title {
  font-size: 15px; font-weight: 700; color: var(--dl-text-1);
...
.dl-tip { max-width: 860px; margin: 24px auto 0; padding: 0 24px; }
.dl-tip-box {
  border-radius: 12px;
  padding: 20px 24px;
  font-size: 13px;
...
.dl-footer {
  ...
  color: var(--dl-text-3); font-size: 13px;
}
.dl-footer a { color: #f97316 !important; text-decoration: none !important; }

.dl-loading { text-align: center; padding: 120px 24px; color: var(--dl-text-2); font-size: 15px; }
.dl-loading-spinner {
  display: inline-block; width: 32px; height: 32px;
  border: 3px solid rgba(249,115,22,0.15);
  border-top-color: #f97316; border-radius: 50%;
  animation: dl-spin 0.8s linear infinite; margin-bottom: 16px;
}
...
<div v-if="!loading" class="dl-hero">
  <h1>Download RsClaw</h1>
  <div class="subtitle">AI Agent Engine with long-term memory and self-learning — OpenClaw-compatible</div>
  <div v-if="version" class="version-badge">{{ version }}</div>

  <!-- Tab switcher -->
  <div style="display:block">
    <div class="dl-tabs">
      <button :class="{ active: activeTab === 'app' }" @click="activeTab = 'app'">Desktop App</button>
      <button :class="{ active: activeTab === 'cli' }" @click="activeTab = 'cli'">CLI Only</button>
    </div>
  </div>

  <!-- Platform detect -->
  <div v-if="detectedOS" class="platform-detect">
    Detected: <strong>{{ detectedOS }} {{ detectedArch }}</strong>
  </div>
  ...
  <div v-if="activeTab === 'app' && appKey" style="margin-top: 16px">
    ...
    <div class="dl-meta-sub">Includes GUI + CLI</div>
  </div>
  ...
  <div class="dl-switch">
    <a href="#all-downloads">See all platforms</a>
    <span v-if="detectedOS === 'macOS'">
      &nbsp;·&nbsp;
      <a v-if="appKey === 'macos-aarch64'" @click.prevent="appKey = 'macos-x86_64'; cliKey = 'x86_64-apple-darwin'">Switch to Intel</a>
      <a v-else @click.prevent="appKey = 'macos-aarch64'; cliKey = 'aarch64-apple-darwin'">Switch to Apple Silicon</a>
    </span>
  </div>
</div>
...
<div v-if="!loading && isWinDownload" class="dl-tip">
  <div class="dl-tip-box dl-tip-win">
    <span class="dl-tip-label">Windows Installation Note</span> — Windows Defender SmartScreen may show a blue "Windows protected your PC" warning, saying the publisher is unknown. This is because the installer is not yet code-signed. The software is safe.
    <div class="dl-tip-detail">
      Click <strong>"More info"</strong> → <strong>"Run anyway"</strong> to proceed with installation.
    </div>
  </div>
</div>
...
<div v-if="!loading" class="dl-install">
  <div class="dl-install-box">
    <div class="dl-install-title">⚡ Quick Install (CLI)</div>
    ...
    <div class="dl-code-wrap">
      <button class="dl-copy-btn" :class="{ copied: copiedIdx === installTab }"
        ...
        :title="copiedIdx === installTab ? 'Copied!' : 'Copy'">
...
<div v-if="!loading" class="dl-section" id="all-downloads">
  <h2 class="dl-section-title">All Downloads</h2>
  <p class="dl-section-desc">Choose the right build for your platform</p>

  <h3 class="dl-section-subtitle">Desktop App (GUI + CLI)</h3>
  <div class="dl-grid">
  ...
  <h3 class="dl-section-subtitle" style="margin-top:28px">CLI Binary</h3>
  <div class="dl-grid">
...
<div v-if="!loading" class="dl-install" style="margin-top:28px">
  <div class="dl-install-box">
    <div class="dl-install-title">🔨 Build from Source</div>
    <div class="dl-code-block" style="border-radius:10px;margin-top:12px">
      <pre><code># Requires Rust 1.91+
